...
class TransolverV1Block(nn.Module):
    ...
    def __init__(
        self, num_heads, hidden_dim, dropout, act="gelu", mlp_ratio=4, last_layer=False, out_dim=1, slice_num=32
    ):
        super().__init__()
...
class TransolverV1(nn.Module):
    ...
    def __init__(
        ...
        if self.unified_pos:
            self.preprocess = MLP(fun_dim + ref * ref, n_hidden * 2, n_hidden, n_layers=0, res=False, act=act)
        else:
...
class SyntheticDrivAerML(Dataset):
    ...
    def __init__(self, n_samples=20, n_points=50000, d_params=6, seed=42):
        ...
        for i in range(n_samples):
            ...
            shear = (
                0.1 * normals * np.abs(coords[:, 0:1]) + params[1] * coords * 0.02 + 0.01 * rng.randn(n_points, 3)
            ).astype(np.float32)
...
def train_and_evaluate(model, train_loader, test_loader, args, device, label):
    ...
    for epoch in range(args.epochs):
        ...
        if test_error is not None:
            ...
            print(f"  Epoch {epoch + 1:3d}/{args.epochs} | train_loss={train_loss:.6f} | time={epoch_time:.2f}s")
...
def parse_args():
    ...
    p.add_argument("--n_points", type=int, default=50000, help="Points per sample (DrivAerML surface has ~8.8M)")
    p.add_argument("--subset_size", type=int, default=10000, help="Amortized training subset size")
    p.add_argument("--epochs", type=int, default=50, help="Training epochs")
    p.add_argument("--lr", type=float, default=1e-3)
    p.add_argument("--weight_decay", type=float, default=0.05)
    p.add_argument("--batch_size", type=int, default=1)
    p.add_argument("--n_layers", type=int, default=8, help="Number of layers (paper uses 24; 8 for quick test)")
    p.add_argument("--n_hidden", type=int, default=128, help="Hidden dim (paper uses 256; 128 for quick test)")
    p.add_argument("--n_head", type=int, default=8)
    p.add_argument("--slice_num", type=int, default=32)
    p.add_argument("--num_tiles", type=int, default=4, help="Tiles for v3 (v1 ignores this)")
...
def main():
    ...
    print(f"  Model config: {args.n_layers} layers, C={args.n_hidden}, H={args.n_head}, M={args.slice_num}")
    ...
    # Train both
    torch.manual_seed(args.seed)
    history_v1 = train_and_evaluate(model_v1, train_loader, test_loader, args, device, "Transolver v1")

    torch.manual_seed(args.seed)
    history_v3 = train_and_evaluate(model_v3, train_loader, test_loader, args, device, "Transolver v3")
    ...
    if v1_peak_mem > 0:
        print(
            f"{'Peak GPU mem (MB)':.<25} {v1_peak_mem:>15.0f} {v3_peak_mem:>15.0f} {v3_peak_mem / v1_peak_mem:>11.2f}x"
        )
