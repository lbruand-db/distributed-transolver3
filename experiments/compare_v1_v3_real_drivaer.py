...
def load_drivaer_vtp(vtp_path, params_csv=None, subsample=None, seed=42):
    ...
    if has_cell_data:
        ...
        # Cell normals
        mesh_with_normals = mesh.compute_normals(point_normals=False, cell_normals=True, auto_orient_normals=True)
        ...
        mesh_with_normals = mesh.compute_normals(point_normals=True, cell_normals=False, auto_orient_normals=True)
...
def compute_target_stats(samples):
    ...
    print(
        f"  Param stats:  mean_range=[{param_mean.min():.1f},{param_mean.max():.1f}], "
...
class RealDrivAerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.stats is not None:
            params = (params - torch.from_numpy(self.stats["param_mean"])) / torch.from_numpy(self.stats["param_std"])
...
def train_model(model, train_loader, args, device, label):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_interval == 0 or epoch == args.epochs - 1:
            print(f"  Epoch {epoch + 1:3d}/{args.epochs} | loss={train_loss:.6f} | time={epoch_time:.2f}s")
...
def render_car_pressure(coords, pressure, title, ax, vmin, vmax, view="top", cmap="RdBu_r", point_size=0.15):
    ...
    sc = ax.scatter(
        x_plot, y_plot, c=pressure, cmap=cmap, norm=norm, s=point_size, alpha=0.9, edgecolors="none", rasterized=True
    )
...
def create_visualizations(coords_3d, gt_pressure, v1_pressure, v3_pressure, out_dir, n_render=50000):
    ...
    fig.suptitle(
        "DrivAerML Surface Pressure: Ground Truth vs Transolver v1 vs v3", fontsize=16, fontweight="bold", y=0.98
    )
    ...
    for row, (view, vlabel) in enumerate(zip(views, view_labels)):
        sc = render_car_pressure(c, gt, f"Ground Truth — {vlabel}", axes[row, 0], vmin, vmax, view=view)
        render_car_pressure(c, v1, f"v1 Prediction — {vlabel}", axes[row, 1], vmin, vmax, view=view)
        render_car_pressure(c, v3, f"v3 Prediction — {vlabel}", axes[row, 2], vmin, vmax, view=view)
    ...
    fig.suptitle("DrivAerML — Side View Pressure Comparison", fontsize=15, fontweight="bold", y=1.02)
    ...
    fig.suptitle("DrivAerML — Top View Pressure Comparison", fontsize=15, fontweight="bold", y=1.02)
    ...
    fig.suptitle("DrivAerML — Absolute Pressure Error (Side View)", fontsize=15, fontweight="bold", y=1.02)
    ...
    fig.suptitle("DrivAerML — Pressure Profiles & Error Distribution", fontsize=14, fontweight="bold", y=1.02)
    ...
    axes[1].hist(
        ...
        alpha=0.6,
        density=True,
        label=f"v1 (mean={np.abs(v1_pressure - gt_pressure).mean():.2f})",
        color="blue",
    )
    axes[1].hist(
        v3_err,
        ...
        alpha=0.6,
        density=True,
        label=f"v3 (mean={np.abs(v3_pressure - gt_pressure).mean():.2f})",
        color="red",
    )
...
def parse_args():
    ...
    p.add_argument("--train_runs", type=int, nargs="+", default=[1, 2], help="Run indices for training")
    p.add_argument("--test_run", type=int, default=3, help="Run index for testing/visualization")
    p.add_argument("--load_subsample", type=int, default=200000, help="Subsample VTP to this many points on load")
    p.add_argument("--train_subset", type=int, default=20000, help="Points per amortized training step")
    ...
    p.add_argument("--render_points", type=int, default=80000, help="Max points for rendering")
    return p.parse_args()
...
def main():
    ...
    test_sample = load_drivaer_vtp(test_vtp, test_csv, subsample=args.load_subsample, seed=args.seed + args.test_run)
